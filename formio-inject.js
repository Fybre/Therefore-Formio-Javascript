// ============================================================
// Therefore Form.io Library — Script Injection Wrapper
//
// Paste this entire file into a Form.io custom action (or
// a shared "On Init" action) to make window.Therefore available
// across all actions on the form.
//
// The injection guard ensures the library is only evaluated once
// per page load, even if the action fires multiple times.
// ============================================================
//
// USAGE
// -----
// Step 1: Paste this entire file into an "On Init" custom action so it
//         runs once when the form loads.
//
// Step 2: In any other custom action on the form, destructure what you need:
//
...
//
// ── Get a client from the current browser session ──────────────────────────
//
...
//   });
//
// ── Save index data (quick — no LastChangeTime required) ───────────────────
//
//   const indexData = new IndexData()
//       .addString('Invoice_No',   data.invoiceNo)
//       .addMoney('Amount',        data.amount)
...
//       categoryNo:     8,
//       conditions:     [ new Condition('Invoice_No', data.invoiceNo) ],
//       selectedFields: ['Invoice_No', 'Amount', 'Status'],
//       maxRows:        100
//   });
//   const result = await client.executeSingleQuery({ Query: query });
//   const rows   = result.QueryResult.ResultRows;   // [{ DocNo, IndexValues }]
//
...
//           { StringIndexData: { FieldName: 'Invoice_No', DataValue: 'INV-001' } }
//       ]},
//       Streams: [{ StreamNo: 0, FileName: 'invoice.pdf',
//                   FileDataBase64JSON: base64, NewStreamInsertMode: 0 }]
//   });
//   const newDocNo = response.DocNo;
//
// ── Get a reference / auto-number ─────────────────────────────────────────
//
//   const refNo = await client.getReferenceNumber('Invoice Reference Numbers');
//
// ── Redraw a Form.io component after data changes ──────────────────────────
//
//   redrawComponent('myDataGridComponent');
//
// ── Full custom action pattern (with error handling) ──────────────────────
//
//   const { ThereforeClient, IndexData, SaveDocumentIndexDataQuickParams,
//           getConfigurationFromLocalStorage, redrawComponent } = window.Therefore;
//   try {
//       const config = getConfigurationFromLocalStorage();
//       const client = new ThereforeClient({ baseUrl: config.apiUrl,
//                                            token: config.token, tenant: config.tenant });
//       const idx = new IndexData()
//           .addString('Submitter', data.fullName)
//           .addDate('Submitted',   new Date());
//       await client.execute('SaveDocumentIndexDataQuick',
//           new SaveDocumentIndexDataQuickParams(data.docNo, idx));
//       redrawComponent('statusLabel');
//   } catch (err) {
//       instance.setAlert('danger', 'Therefore error: ' + err.message);
//   }
//
// ============================================================
...
class Condition {
     ...
     * @param {string} condition - The condition string (e.g., ">= 100", "LIKE Value%").
...
class ThereforeClient {
     ...
     * @param {object} data - The data object to send in the body.
     ...
     *
     * Resolves the table name to DataTypeNo via GetObjects, derives CategoryNo from
     * the TableName field in GetReferencedTableInfo (e.g. "TheCat43" -> 43),
     * then pages through results using ExecuteAsyncSingleQuery / GetNextSingleQueryRows.
     *
     * @param {string} tableName - Referenced table name (case-insensitive).
     * @param {Array} [conditions] - e.g. [{ FieldNoOrName: 'Entity', Condition: '*' }]
     * @param {object} [options]
     * @param {number} [options.maxRows=5000]
     * @param {number} [options.blockSize=1000]
     * @returns {Promise<{dataTypeNo, categoryNo, name, columns, rowCount, rows}>}
     */
    async queryReferencedTable(tableName, conditions, options) {
        conditions = conditions || [];
        var maxRows   = (options && options.maxRows   != null) ? options.maxRows   : 5000;
        var blockSize = (options && options.blockSize != null) ? options.blockSize : 1000;

        // 1. Discover DataTypeNo by name (Type 5 = referenced tables)
        var objectsResp = await this.execute('GetObjects?limit=1000&skip=0', { Flags: 0, Type: 5, PermType: 8 });
        var items = objectsResp.ItemList || objectsResp.Items || [];
        var nameLower = tableName.toLowerCase();
        var match = null;
        ...
        var dataTypeNo = Number(match.ID);

        // 2. Get table info — parse CategoryNo from TableName (e.g. "TheCat43" -> 43)
        var tableInfo = await this.execute('GetReferencedTableInfo', { DataTypeNo: dataTypeNo });
        var tableNameStr = tableInfo.TableName || '';
        var catMatch = tableNameStr.match(/\\d+$/);
        if (!catMatch) throw new Error('Cannot parse CategoryNo from TableName: ' + tableNameStr);
        var categoryNo = Number(catMatch[0]);

        // 3. Start async query — first response includes QueryId and first page
        var firstResp = await this.execute('ExecuteAsyncSingleQuery', {
            Query: {
                CategoryNo: categoryNo,
                Conditions: conditions,
                MaxRows: maxRows,
        ...
        var queryId = firstResp.QueryId || firstResp.QueryID;
        var firstRows = (firstResp.QueryResult && firstResp.QueryResult.ResultRows) || [];
        var rows = firstRows.slice();
        var hasMore = Boolean(firstResp.HasRemainingRows);

        // 4. Page through remaining rows
        try {
            while (hasMore && queryId != null) {
                var nextResp = await this.execute('GetNextSingleQueryRows', {
                    QueryID: queryId,
                    RowBlockSize: blockSize,
                });
                var nextRows = (nextResp.QueryResult && nextResp.QueryResult.ResultRows) || [];
                for (var j = 0; j < nextRows.length; j++) { rows.push(nextRows[j]); }
                hasMore = Boolean(nextResp.HasRemainingRows);
            }
        } finally {
            if (queryId != null) {
                this.execute('ReleaseSingleQuery', { QueryID: queryId }).catch(function() {});
            }
        }
...
/**
 * Utility to convert a File/Blob to Base64 string for StreamData.
 * @param {File|Blob} fileData - The file object or blob.
...
// INJECT THEREFORE SCRIPT ONLY ONCE
